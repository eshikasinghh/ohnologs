...
for specie in species_list:
    ...
    unique_chromosomes = df['gene_biotype'].unique()
    gene_biotype_dict[specie] = unique_chromosomes
#   print(unique_chromosomes)

df_chromosomes = pd.DataFrame(dict([(k, pd.Series(v)) for k, v in gene_biotype_dict.items()]))
    # k = specie name
    # v = list of unique chromosomes
    #converts dictionary into datagrame

col_width = max(len(col) for col in df_chromosomes.columns) + 10
    #calculated length of the longest species name and adds 2 for extra space/readability

with open('unique_gene_biotype.txt', 'w') as f: #opens file in write mode
